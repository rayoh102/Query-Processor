...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  uint8_t byte;
  int bytesLeft = doctable_bytes + memidx_bytes;
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    LinkedList* curr_list = (ht->buckets)[i];
    int list_length = LinkedList_NumElements(curr_list);
    int rsize = WriteHTBucketRecord(f, record_pos, list_length, bucket_pos);

    if (rsize == kFailedWrite) {
      return kFailedWrite;
    }
    ...
    if (list_length != 0) {
      int bucket_bytes = WriteHTBucket(f, bucket_pos, curr_list, fn);
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int size = fn(f, element_pos, &key_val);
    if (size == kFailedWrite) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  char* fname = static_cast<char*>(kv->value);
  int16_t file_name_bytes = strlen(fname);  // you may want to change this
  ...
  if (fwrite(fname, file_name_bytes, 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    LLPayload_t curr;
    LLIterator_Get(it, &curr);

    // STEP 14.
    // Truncate to 32 bits, then convert it to network order and write it out.
    position.position = reinterpret_cast<intptr_t>(curr);
  ...
  return sizeof(DocIDElementPosition) * num_positions +
    sizeof(DocIDElementHeader);   // you may want to change this
}
