...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      ElementPositionRecord e_pos;
      Verify333(fread(&e_pos,  sizeof(DocIDElementPosition), 1, file_) == 1);
      e_pos.ToHostFormat();
      Verify333(fseek(file_, e_pos.position, SEEK_SET) == 0);
