...
// The function gets a keyvalue pair given a key and its chain and also
// potentially removes the key
//
// Arguments:
// chain: the LinkekdList chain in which we will be searching
// key: the key that we will be searching for
// removeKey: a boolean that represents whethber or not we will remove the key in the chain
//
// Returns:
// a Pointer to the key (returns NULL if the key cannot be found)
static HTKeyValue_t* getKey(LinkedList* chain, HTKey_t key,bool removeKey);
...
bool HashTable_Insert(HashTable *table,
  ...
  HTKeyValue_t* keyExists = getKey(chain, newkeyvalue.key, false);
  //If key doesn't exist, add
  if (keyExists == NULL) { 
    keyExists = malloc(sizeof(HTKeyValue_t));
    *keyExists = newkeyvalue;
    LinkedList_Append(chain, (LLPayload_t)keyExists);
    oldkeyvalue = NULL;
    table -> num_elements += 1;
    return false;  
  } 
  else { // If key exists, then update
    *oldkeyvalue = *keyExists;
    *keyExists = newkeyvalue;
    return true;
  }
}
...
static HTKeyValue_t* getKey(LinkedList* chain, HTKey_t key, bool removeKey) {
  LLIterator *nodeItr = LLIterator_Allocate(chain);  
   
  while (LLIterator_IsValid(nodeItr )) {
    HTKeyValue_t *keyvalue;
    LLIterator_Get(nodeItr, (LLPayload_t*)&keyvalue); 
      if (keyvalue -> key == key) { 
      if (removeKey) { 
...
bool HashTable_Find(HashTable *table,
  ...
  HTKeyValue_t* kv = getKey(chain, key, false); 
...
bool HashTable_Remove(HashTable *table,
  ...
  int bucket = HashKeyToBucketNum(table, key);
  LinkedList *chain = table->buckets[bucket];
  HTKeyValue_t* removedKey = getKey(chain, key, true);
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  if (HTIterator_IsValid(iter)) { 
    bool next = LLIterator_Next(iter->bucket_it);  
    if (!next ) {  
      int curreIdx= iter->bucket_idx;
      int numBuckets = iter->ht->num_buckets;
      for (int i = curreIdx+ 1; i < numBuckets; i++) {
        if (LinkedList_NumElements(iter->ht->buckets[i])) { 
	  LLIterator_Free(iter->bucket_it);  
	  iter->bucket_idx = i;
