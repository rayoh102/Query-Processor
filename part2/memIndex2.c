...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  if (HashTable_Find(index, key, &kv) != 1) {
    ...
    while (HTIterator_IsValid(itr)) {
      ...
      search ->doc_id = (DocID_t) kv.key;
      search ->rank = LinkedList_NumElements((LinkedList*) kv.value);
      LinkedList_Append(ret_list, (LLPayload_t) search);
      HTIterator_Next(itr);
    }
    HTIterator_Free(itr);
  }
  ...
  for (i = 1; i < query_len; i++) {
    ...
    search = HashTable_Find(index, FNVHash64((unsigned char *) query[i], strlen(query[i])), &kv);
    if (search == 0) {
      LinkedList_Free(ret_list, (LLPayloadFreeFnPtr) &free);
      return NULL;
    }
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      if (HashTable_Find(wp->postings, search->doc_id, &kv) == 1) {
        search->rank += LinkedList_NumElements((LinkedList*) kv.value);
