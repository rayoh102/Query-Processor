...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (1) {
    if (*cur_ptr == '\0') {
      break;   // No more valid char in string
    }
    if (isalpha(*cur_ptr) == 0) {
      // Char is a non-alphabetic character
      *cur_ptr = '\0';
      // Check previous char to see if valid char
      // If yes, add completed string to hashtable
      if (isalpha(*(cur_ptr - 1)) != 0 && cur_ptr != content) {
        AddWordPosition(tab, word_start, word_start - content);
      }
      word_start = cur_ptr + 1;
    } else {
      // Char is an alphabetic character
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    // Create string for wp
    char* word_str = (char*) malloc(strlen(word) + 1);
    Verify333(word_str != NULL);

    // Copy word to char pointer
    strncpy(word_str, word, strlen(word) + 1);
    ...
    // Initialie fields
    wp->word = word_str;  // initialize word field

    wp->positions = LinkedList_Allocate();  // initialize linkedlist positions
    ...
    int retval = HashTable_Insert(tab, kv, &old_kv);
    Verify333(!retval);
  }
}
