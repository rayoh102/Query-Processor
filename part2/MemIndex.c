...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  while (HTIterator_IsValid(iter)) {
    ...
    SearchResult* temp_search = (SearchResult*) malloc(sizeof(SearchResult));
    Verify333(temp_search != NULL);
    temp_search->doc_id = kv.key;
    temp_search->rank = LinkedList_NumElements(kv.value);
    LinkedList_Append(ret_list, temp_search);
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      ...
      LLIterator_Get(ll_it, (LLPayload_t*) &current_item);
      if (!HashTable_Find(wp->postings, current_item->doc_id, &kv)) {
        // Can't find word in file, removing file from consideration
        LLIterator_Remove(ll_it, &free);
      } else {
        // Found word in file, increase rank
        current_item->rank += LinkedList_NumElements(kv.value);
