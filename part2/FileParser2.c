...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (result == -1) {
    perror("Couldn't read file stats\n");
    return NULL;
  }
  ...
  if (!S_ISREG(file_stat.st_mode)) {
    fprintf(stderr, "File isn't a regular type\n");
    return NULL;
  }
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  // variable to indicate whether we are inside a word
  bool inWord = false;

  while (1) {
    // convert to lowercase 
    *cur_ptr = tolower(*cur_ptr);
    char currChar= *cur_ptr;

    // if current character is alphanumeric and we aren't currently inside a word
    if (isalpha(currChar) && !inWord ) {
      // set word_start as current character
      word_start = cur_ptr;
      // set inWord to true
      inWord = !inWord ;
    } else {
      *cur_ptr = '\0';
      if (word_start && inWord) {
